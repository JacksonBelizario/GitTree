...
const mapToProps = (state : IStore) : StoreProps => ({
    folder: state.folder, currentBranch: state.currentBranch,
}); 
    ...
    return (
        <Navbar>
            <NavbarGroup align={Alignment.LEFT}>
                ...
                { currentBranch && <Tag icon={<GitBranchIcon size={18} />} large minimal style={{marginLeft: 10}}>{currentBranch.name}</Tag> }
            </NavbarGroup>
        </Navbar>
    );
}
