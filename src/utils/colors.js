...
export const getMaterialColors = (text, shade = 500) => {
  ...
  const hashedText = Math.abs(hashCode(text));
  const colorIndex = hashedText % materialColors.length;
  const colorObj = materialColors[colorIndex];

  return {
    backgroundColor: colorObj.shades[shadeStr],
    color:
      parseInt(shadeStr, 10) >= colorObj.whiteBreakpoint
        ? materialWhite
        : materialBlack,
...
export const generateMaterialColor = (text, shade = 500) => {
